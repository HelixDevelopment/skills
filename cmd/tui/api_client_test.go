...
// — and asserts the TUI client's outgoing request carries the key in that exact
...
func TestRequestSendsServerCanonicalAPIKeyHeader(t *testing.T) {
	...
	c := NewAPIClient(srv.URL, wantKey)
	c.client = srv.Client()

	resp, err := c.request(context.Background(), http.MethodGet, "/api/v1/ping", nil)
	if err != nil {
		t.Fatalf("TUI client failed to authenticate against a server that reads X-API-Key: %v "+
